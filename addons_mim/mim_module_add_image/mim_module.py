...
class mim_wizard(osv.osv_memory):
    ...
    def order_line_create(self, cr, uid, ids, context=None):
	    ...
	    sale_order_line_obj = self.pool.get('sale.order.line')#on recupÃ¨re l'objet sale.order.line pour l'utiliser avec la fonction ORM create 
	    ...
	    sale_order_line_obj.create(cr, uid, {'product_id':select_type.id,'name': name,'order_id':order_id, 'product_uom_qty': rec_qty, 'price_subtotal' : total['totalcacher'], 'price_unit': total['totalcacher']/rec_qty,'image':res_image})
            return True
...
class sale_order(osv.osv):
    ...
    ####permet la creation de la nouvelle fenetre pop-up###
        return {
            'name': 'Mim Wizard',
...
Si cette offre vous convient, merci de nous la renvoyer signée avec la mention "Bon pour accord"'''
    ...
    def convNombre2lettres(self, cr, uid, ids, nombre, context=None):
    ...
    def capitaliser (self, cr, uid, ids, chaine,context=None):
        s = chaine.lower()
        s0 = s[0]
        s0 = s0.upper()
        s = s0 + s[1:] 
    ...
    def traitement_virgule (self, cr, uid, ids, nombre,context=None):
        s=''
        lettre0 = str(nombre)
        ...
        pricelist_id = self.browse(cr, uid, ids, context=context)[0].pricelist_id.id
        currency_name = self.pool.get('product.pricelist').browse(cr, uid, pricelist_id, context=context).currency_id.currency_name
        currency_name = self.capitaliser(cr, uid, ids, currency_name, context)
        
        lettre_ent = self.convNombre2lettres(cr, uid, ids, entiere2, context)
        lettre_dec = self.convNombre2lettres(cr, uid, ids, decimal2, context)
