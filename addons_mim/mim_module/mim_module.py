...
class article_categorie(osv.osv):
...
class mim_wizard(osv.osv_memory):
	...
	'select_type': fields.many2one('product.product', 'Type', domain=[('categ_id','=',3)], change_default=True),#,domain=[('categ_id','=','Types')]
   ...
   #pour d̩finir les valeurs par d̩faut afficher sur la fenetre pop-up
    _defaults = {
    ...
    def order_line_create(self, cr, uid, ids, context=None):
	    ...
	    name = lxh
	    sale_order_line_obj.create(cr, uid, {'product_id':select_type.id,'name': name,'order_id':rec_ref+1, 'product_uom_qty': rec_qty, 'price_subtotal' : total['totalcacher'], 'price_unit': total['totalcacher']/rec_qty})
            return True
...
class sale_order(osv.osv):
    ...
    _inherit = "sale.order"
    _columns = {'entete': fields.text('Sujet'),
    ...
    def action_mim_wizard(self, cr, uid, ids, context=None):
        ...
        res = mod_obj.get_object_reference(cr, uid, 'mim_module', 'view_mim_wizard')
        res_id = res and res[1] or False,
        ctx = dict()
        sujet = self.browse(cr, uid, ids, context=context)[0].name
        order_ref = self.browse(cr, uid, ids, context=context)[0].name
        order_ref = sujet[2:]
        ctx.update({
